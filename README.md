...
remote/          本机远程控制扩展 (**受控端仅 Windows**, 鼠标键盘 + 截屏, WS / gRPC / PeerJS 三传输)
  ...
  ├─ calibrate.py  坐标校准 (闭环实测图坐标↔鼠标坐标, 纯 Pillow)
  ├─ window.py     窗口查询与切前台 (纯 ctypes: list / foreground / focus, 仅 Windows)
  ...
  ├─ client.py     命令行客户端 (单机调试级: 一条命令打一台一个 op + locate 视觉定位 + calibrate 坐标校准)
...
docs/knowledge/               **踩坑与注意事项, 一个文件一个知识点 (65 条), 见 docs/knowledge/README.md**
