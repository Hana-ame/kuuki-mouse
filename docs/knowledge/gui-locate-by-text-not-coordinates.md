...
## 找到就点：合成一次调用 (`screen.click_text`)

`find_text` 给坐标、`click_text` 连点也做了。合成的理由不是省一次往返：**分成两次
调用的话，找与点之间隔着一整个来回，屏幕可能已经变了**（会滚动的列表、会刷新的
页面），点在"刚才那一帧的坐标"上，点中的可能已经不是那一行。

与 `find_text` 不同的三条口径：
- **没找到要报错**（`not_found`）：查询可以空手而归，动作不行 —— 静默回个
  `found=false` 会被调用方当成"点了"。
- `index` 选第几个匹配，越界报 `bad_request`（不退化成"点第一个"）。
- `dry_run` 不点，但**点击参数照常回显** —— 否则调用方以为看清了将要发生什么。

点击默认值（`button` / `clicks` / `interval` / `hold`）抽成 `_click_plan()`，
`mouse.click` 与 `click_text` 共用一份：两处各写一份的话，"改了默认 hold 只改了
一边"根本进不了比对（两条传输一起错，看上去是一致的）。
