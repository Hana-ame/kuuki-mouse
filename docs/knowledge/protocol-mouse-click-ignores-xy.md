...
| proto `ClickMouseRequest` | 新增 `optional int32 at_x / at_y`（必须 optional，见 `protocol-proto3-optional.md`） |
| `remote/grpc_server.py` / `remote/client.py` | `HasField("at_x")` 判断后透传；客户端 `_optional_at()` 只在显式给坐标时写入 |

**为什么字段必须 optional**：直接 `at_x=args.get("x", 0)` 会把「没给坐标」变成
「定位到 x=0」—— 点在屏幕左上角。显式 0 又是合法坐标，所以只能用 optional +
`HasField`。
