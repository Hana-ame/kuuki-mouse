...
class Negotiator:
    ...
    async def _makeOffer(self):
        ...
        provider = self.connection.provider
        try:
            # aiortc 的 createOffer() 不接受参数 (JS 版传 options.constraints)
            offer = await peerConnection.createOffer()
            log.info("Created offer.")

            sdp_transform = self.connection.options.get("sdpTransform") or None
            if sdp_transform and callable(sdp_transform):
                tSDP = sdp_transform(offer.sdp)
                if tSDP:
                    offer.sdp = tSDP
            try:
